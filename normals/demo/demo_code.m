...
img_data = {'1.png','demo/img_000001.jpg', 'demo/img_000002.jpg'};
nn=0;
...
for i = 1:length(img_data)
	...
	%
        ...
        snd(:,:,:,1) = (1/sqrt(3))*ones(cnn_input_size+nn*2, cnn_input_size+nn*2,3);
        depd(:,:,:,1) = imresize(imread('11.png')/255, [cnn_input_size, cnn_input_size], ...
                           'bilinear', 'antialiasing', false);
        

        %
        net.blobs('data0').reshape([crop_height+nn*2, crop_width+nn*2, 3, 1]);
        net.blobs('data2').reshape([crop_height+nn*2, crop_width+nn*2, 1, 1]);
        net.blobs('data1').reshape([crop_height+nn*2, crop_width+nn*2, 3, 1]);


        input_data = zeros(crop_height+nn*2,crop_width+nn*2,3,1);
        input_data((1+nn):crop_width+nn, (1+nn):crop_width+nn, :, 1) = ims;
        depd((1+nn):crop_width+nn, (1+nn):crop_width+nn, :, 1) = 1;
